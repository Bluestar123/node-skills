...
// 1. 生成 tokens
/**
 * 第一步: 将代码解析为tokens。这个过程需要tokenzier(分词器)函数，
 * 整体思路就是通过遍历字符串的方式，对每个字符按照一定的规则进行switch case，
 * 最终生成tokens数组 input 代码字符串
 * 词法分析 - 得到格式如下：
 * [
      { type: "主语", value: "你" },
      { type: "谓语", value: "是" },
...
/**
 * 第二步: 将tokens数组转化为AST
 * 现在需要定义parser函数，接收上一步处理好的tokens
 *
 * 语法分析生成 ast
 */
...
//3.遍历和访问生成好的AST
/**
 * 现在已经有AST了，然后我们希望能够通过访问器访问不同的节点，
 * 当遇到不同的节点的时候，调用访问器的不同函数
 */
...
//4.Transformer转化
/**
 * 现在已经生成好AST了。在这一步需要使用到转换器，
 * 帮助我们将刚才生成的AST转化为新的AST。在转化之前，必须要明确转化后的AST长什么样
...
console.log(compiler(input)) // add(2, subtract(4, 2))
